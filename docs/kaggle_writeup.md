...
**Architecture** (see cover image). A team of ADK `LlmAgent`s (coordinator → researcher, writer) runs under a Runner. The governor is a **Runner plugin** — registered once, its callbacks cover every LLM call of every agent *and every spawned subagent*, which is exactly the semantics a governor needs: a per-agent callback could be bypassed by a spawned child; a Runner plugin cannot.
